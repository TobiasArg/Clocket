# backend-owned-goal-progress-and-entry-resolution Specification

## Purpose
Define backend-owned goal progress read models and linked-entry resolution behavior so canonical goal totals, metadata propagation, and delete resolution policies are produced by the backend while the frontend remains presentation-only.
## Requirements
